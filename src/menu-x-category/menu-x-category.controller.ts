...
export class MenuXCategoryController {
  ...
  @Patch(':mid/:cid')
  async update(@Param('mid') mid: string, @Param('cid') cid:string, @Body() updateMenuXCategoryDto: UpdateMenuXCategoryDto) {
    let result:menu_X_category;
    result = await this.menuXCategoryService.update(+mid, +cid, updateMenuXCategoryDto);
    return result;
  }
