...
export class CategoryXItemController {
  ...
  @Put(':cid/:iid')
  async update(@Param('cid') cid: string, @Param('iid') iid: string, @Body() updateCategoryXItemDto: UpdateCategoryXItemDto) {
    let result:category_X_item;
    result = await this.categoryXItemService.update(+cid, +iid, updateCategoryXItemDto);
    return result;
  }
