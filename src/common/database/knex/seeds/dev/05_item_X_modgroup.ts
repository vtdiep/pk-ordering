...
export async function seed(knex: Knex): Promise<void> {
    ...
    // Get item ids
    let items = await knex<Item>('item').select().where('is_standalone', "=", true)
    console.log(items);
    ...
    // Omelettes
    pickMods = ["Salt"]
    pickItems = ["Ham & Cheese Omelette", "Avocado Cheese Omelette", "Cheese Omelette"]
    modgroupIDs = await knex<Modgroup>('modgroup').select().whereIn('name', pickMods).pluck('mod_id')
    itemIDs = await knex<Item>('item').select().where('is_standalone', "=", true).whereIn('name', pickItems).pluck('item_id')
    pairings = cartesianProduct(modgroupIDs)(itemIDs)
    pairedObjects = pairings.map(x=>{
        return {mod_id:x[0], item_id:x[1], item_is_standalone:true, display_order: display_order++}
    })
