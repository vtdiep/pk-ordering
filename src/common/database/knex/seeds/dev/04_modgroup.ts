...
export async function seed(knex: Knex): Promise<void> {
    ...
    // Omelettes
    modgroups.push(
        {name: "Salt", required_selection: 0, max_selection: 1, max_single_select: 1, free_selection:1, private_note: ""}
    )
    options.push(
        [{name: "No Salt", description: "", active: true, is_standalone: false, price: 0, private_note: "" }]
    )
    ...
    modgroups.push(
        {name: "Choose a topping", required_selection: 0, max_selection: 3, max_single_select: 3, free_selection:1,
            price: 1, description: "", private_note: "5/10/21"}
    )
    options.push([
    ...
    for (let i = 0; i < mod_ids.length; i++) {
        ...
        for (let j = 0; j < item_ids.length; j++) {
            await knex("modgroup_item").insert([
                {mod_id: mod_id, item_id: item_ids[j], item_is_standalone: false,  display_order: display_order++}
            ])
